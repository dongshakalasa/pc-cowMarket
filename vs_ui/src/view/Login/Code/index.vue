  ...
  methods: {
    ...
    async getCode() {
      ...
      if (regEmail.test(this.account)) {
        const result = await this.$API.user.reqGetCodeLogin(this.account);
        if (result.code == 200) {
    ...
    async sendLogin(){
      ...
      const result = await this.$API.user.reqUserCodeLogin(data)
      console.log(result);
      if(result.code == 200) {
        setToken(result.data.token);
        this.$message({message:result.message,type:"success"})
        this.$bus.$emit('sendAccount',result.data.account)
